...
이 서비스는 `@sentry/bun`으로 예상하지 못한 서버 오류와 HTTP 5xx만 수집한다. 정상적인 4xx, 요청·사용자 정보, 오류 원문은 전송하지 않는다. 배포 시 GitHub Environment `team-mino-prod`의 `SENTRY_DSN`과 Git SHA 기반 `SENTRY_RELEASE`가 Cloud Run에 주입된다.

## 최초 설정

1. Sentry 조직에서 Bun 플랫폼의 `team-mino-api` 프로젝트를 만든다.
2. 프로젝트 DSN을 GitHub Environment `team-mino-prod`의 `SENTRY_DSN` secret으로 등록한다. DSN을 저장소, 이슈, PR 본문에 붙이지 않는다.
...
알림만 중지하려면 Sentry workflow를 비활성화한다. 수집도 중지하려면 Cloud Run에서 `SENTRY_DSN`을 제거하고 재배포한다. Discord 앱 제거는 다른 Sentry 프로젝트가 같은 서버 연동을 사용하지 않는지 확인한 뒤 수행한다.
